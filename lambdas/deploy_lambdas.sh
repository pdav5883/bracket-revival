...
for dir in $dirs; do
    ...
    # Skip 'common' directory that contains lambda layer, and dirs starting with . or _
    if [[ "$lambda_short_name" == *"common" || "$lambda_short_name" == "dev" || "$lambda_short_name" == .* || "$lambda_short_name" == _* ]]; then
        continue
    fi
